...
iso.isotope(isotopeAttrs);}).eq(0).trigger("click")}});$(function(){isNoviBuilder=window.xMode;function getSwiperHeight(object,attr){var val=object.attr("data-"+attr),dim;if(!val){return undefined;}
...
function attachFormValidator(elements){regula.custom({name:'PhoneNumber',defaultMessage:'Invalid phone number format',validator:function(){if(this.value==='')return true;else return /^(\+\d)?[0-9\-\(\) ]{5,}$/i.test(this.value);}});for(var i=0;i<elements.length;i++){var o=$(elements[i]),v;o.addClass("form-control-has-validation").before("<span class='form-validation'></span>");v=o.parent().find(".form-validation");if(v.is(":last-child"))o.addClass("form-control-last-child");}
...
if(plugins.jscolor.length){jscolorInit();}
if(plugins.themeSwitcher.length){document.documentElement.addEventListener('theme-switching',function(){loaderTimeoutId=setTimeout(function(){plugins.preloader.removeClass("loaded");},500);});document.documentElement.addEventListener('theme-switched',function(event){clearTimeout(loaderTimeoutId);setTimeout(function(){if(windowReady){plugins.preloader.addClass("loaded");}},400);if(plugins.themeCheckbox.length){plugins.themeCheckbox.checked=(plugins.themeCheckbox.getAttribute('data-theme-checkbox')===event.switcher.active);}});document.documentElement.addEventListener('theme-color-change',function(event){document.querySelector(event.switcher.selectors.color+'[name="'+event.variable+'"]').style.backgroundColor=event.value;});var switcher=themeSwitcherInit({variablesFallback:isIE,cookie:false,themes:{"default":{styles:'css/style.css'},"style-1":{styles:'css/style-1.css'},"style-2":{styles:'css/style-2.css'},"style-3":{styles:'css/style-3.css'},"style-4":{styles:'css/style-4.css'},"style-5":{styles:'css/style-5.css'},"style-6":{styles:'css/style-6.css'},"style-7":{styles:'css/style-7.css'},"style-8":{styles:'css/style-8.css'}}});}
if(plugins.themeSwitcher.length&&plugins.themeCheckbox.length){plugins.themeCheckbox.addEventListener('change',function(event){if(this.checked){switcher.setTheme(this.getAttribute('data-theme-checkbox'));}
else{switcher.setTheme(switcher.initial);}});}
if(plugins.buttonNina.length&&!isNoviBuilder){initNinaButtons(plugins.buttonNina);}
$(".modal").on('hidden.bs.modal',function(){$(".modal iframe").attr("src",$(".modal iframe").attr("src"));$("body").css('position','static');});if(isMobile){$(".modal").on('show.bs.modal',function(){$("body").css({'position':'fixed','top':'0','left':'0','right':'0','bottom':'0'});});}
if(!isNoviBuilder&&plugins.typedjs.length){var typed=new Typed(".typed-text",{stringsElement:".typed-strings",typeSpeed:80,loop:!0,backDelay:1500,backSpeed:80});}
if(plugins.projectToggle.length){plugins.projectToggle=$('[data-project-toggle]');plugins.projectToggle.on('click',function(event){event.preventDefault();event.stopPropagation();$(this).parents('.box-project-social').find('.box-project-icon-list').toggleClass('active');$(this).toggleClass('active');});$body.on('click',function(){$('.box-project-icon-list').removeClass('active');plugins.projectToggle.removeClass('active');});}
var parallaxJs=document.getElementsByClassName('parallax-scene');if(parallaxJs&&!isNoviBuilder){for(var i=0;i<parallaxJs.length;i++){var scene=parallaxJs[i];new Parallax(scene);}}
...
if(plugins.vide.length){for(var i=0;i<plugins.vide.length;i++){var $element=$(plugins.vide[i]),videObj=$element.data("vide").getVideoObject();videObj.play();if(isNoviBuilder||!isScrolledIntoView($element)){videObj.pause();}
...
$('#section-ripples-1').ripples({resolution:512,perturbance:0.01,});$('#section-ripples-2').ripples({resolution:512,perturbance:0.01,});setInterval(function(){var $el=$('#section-ripples-1, #section-ripples-2');var x=Math.random()*$el.outerWidth();var y=Math.random()*$el.outerHeight();var dropRadius=20;var strength=0.01+Math.random()*0.04;$el.ripples('drop',x,y,dropRadius,strength);},3000);if(plugins.tiltHover.length){plugins.tiltHover.tilt({maxTilt:5,glare:true,maxGlare:.5})}});}());